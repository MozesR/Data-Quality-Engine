...
def markdown_to_lines(text: str) -> list[str]:
    ...
    for raw in text.splitlines():
        ...
        if in_code:
            prefix = '    '
            lines.extend(wrap_line(prefix + line, 95))
            continue
...
def build_pdf(pages: list[list[str]]) -> bytes:
    ...
    pages_kids_refs = []
    # placeholder for pages object index
    ...
    pages_dict = (
        f"<< /Type /Pages /Count {len(page_obj_ids)} /Kids [{' '.join(pages_kids_refs)}] >>"
    ).encode('latin-1')
    objs[pages_obj_id - 1] = pages_dict
